# this R script is writen by the author of the sci article. and read and noted by me.
# 因为刚刚使用，对github还不太熟悉，不知道批注会不会影响到被fork的库，所以就不fork原代码库了（侵删）
# 所有中文批注为本人学习添加的批注/笔记
# because of i am a beginner of the github, don't know the fork feature very well, avoid to broke something, temporarily haven't fork the original repository.
# 学习计划：第一遍 通读一遍，了解大概内容，随心看
#           第二遍 把每一小段总结成提纲
#           第三遍 根据第二遍写出来的提纲，自己写代码实现（不要求跟源代码一致）
...
# 加载tidyverse核心包
library(tidyverse)
# 加载tidyverse中用于处理字符串的包
library(stringr)
# 加载用于处理时间的包
library(lubridate)
# 加载dplyr，这个包好像是属于tidyverse核心包，好像没有必要专门加载
library(dplyr)
# 加载用于生成表格的包
library(gtsummary)
# Ivs(即“eye-vee”)是一个专门用于以通用方式处理间隔的包。暂时不知道干啥的，先机翻一下。
library(ivs)

########################################################################################################################
##### STEP 1: Connect with SQL database -----
# 建立用于查询mimiciv_icu的链接
...
# 导出所有icustays表格内的信息
# tbl生成的是一个虚拟表，当后面请求数据的时候才会获取，collect为（强制）请求数据的命令。
# 这样获取表格，因为本研究研究的是所有ICU的患者，所以直接collect(),如果是研究一部分的话，这样可能有点浪费内存了。
df_icustays_all <- tbl(mimic_icu, "icustays") %>% collect()

# Inclusion flowchart numbers
# 获取一共多少次icustays和多少个患者。这里使用了一个data.frame，然后每一列保存一个数据，这个的好处是不会建立太多的变量，以至于查看的时候头晕眼花，这个df相当于一个变量夹了，值得借鉴。
...
# 筛选出icu入住超过4天的患者（原文说是total length超过4天，但是这个代码好像没有累加）
# 这里生成了一个数据集，相当于是一边统计患者人数，一边生成数据集，每一步都有一个新的数据集（方便处理）
df_icustays_subject_selection <- df_icustays_all %>% filter(los >= 4) %>% collect()

# Inclusion flowchart numbers
# 统计出每一步的患者数，并统计患者人数，然后一起放进一个表格中，方便最后一起做流程图
# 制作流程图应该是先设计好想要的形式，然后统计数据的时候按照涉及好的流程来进行
# 不同的顺序的话，每一步排除出去的患者数就不一样了
...
# Inclusion flowchart numbers
# 根据刚才排除完之后的步骤，再次收集各纳排步骤中的患者数量（这里是否可以写一个函数？自定义函数是不是不便于阅读？）
# df其实也可以当成类似于字典的使用，列名相当于key，值就是值。使用list with name应该也是一样的（r中还不会用）。
# 这个命名，之前的los4后面再增加readm，以明确表示。这个readm是不是应该写出dereadm？
...
# 主语在最前面，ids，然后后面修饰，以明确表达变量的内容
# 这里为啥突然只保留id了？因为患者信息的筛选已经结束了，后面要进行血糖的，需要重新查询，而重新查询，我们只需要id就可以了
...
# 依然是先获取整个表格。对于比较小的表格直接获取可能效率更高（可以直接在r里写代码进行处理）。
dfItems <- tbl(mimic_icu, "d_items") %>% collect() # Table with definition of itemids (e.g. types of nutrition)
# grepl("nutrition", tolower(category)，这个就相当于sql里里面的lower（category）like "%nutrition%"
# 此处使用grepl是在r中运行，并没有转sql，这就是直接collect的好处。
# grepl函数来自base R，是否影响其在转sql过程中的准确性？建议查tidyverse里转sql的官网查看推荐代码
...
# inputevents表格是相当大的，因此一定要进行适当的filter，之后再获取。对于代码书写，最好先show一下sql代码再进行，避免获取太大数据。
df_nut_all <- tbl(mimic_icu, "inputevents") %>% 
  # 这里先进行了collect()，如果不进行的话，后面的是不是会转化为sql然后再运行？孰优孰劣？各有优劣吧，先转化为数据框在r内运算效率肯定低，但是code难度低一些，也不易错
  filter(stay_id %in% stayids_incl_subject_selection & itemid %in% itemid_nut & amount > 0) %>%  collect() %>% 
  # Add feeding label
  # 这里先想好一共需要几个表，每个表需要什么内容，最后合并。但是也不能一直想，肯定是边想边做是最对的。
  ...
  # 把时间都转化为小时，这样单位就统一了。为啥要以第一天的凌晨来算呢？
  ...
  # 上一个group也是by stay_id,这一次也是，为什么要先ungroup在group呢？
  ungroup() %>% group_by(stay_id) %>% arrange(stay_id, starttime_seq) %>% 
  # 最后一条没看懂，需要运行一下看看结果是啥，今天不想看了暂时
...
df_nut_all <- df_nut_all%>% group_by(stay_id) %>%
  # 前三条就是只进行了intake、enteral或者parenteral的情况。如果混合有其他种就是混合的，如果混合有Nutrition - Supplements，就忽略。
...
# 上边那么多都是为了挑出来进行了肠内营养的患者。这样的话，直接挑选出来进行了肠内的不就行了么？
# 原文中先写的没有血糖，但是代码是先判断的没有肠内
...
# 本研究涉及了这么多的表，应该如何记住/画出他们之间的关系呢？否则记不住啊
df_gluc_all <- tbl(mimic_icu, "chartevents") %>% 
  # 筛选过后立马获取数据框，之后就可以在r内处理了。
...
# 把icustay的血糖筛选出来（这个地方为啥不转为sql后进行？其实上边查询数据的时候已经筛选了一次了。奥，因为血糖测量只有subject id，没有stay id）
df_gluc_included_icustays <- df_gluc_all %>% filter(stay_id %in% stayids_incl_nut)

## Add to subject flowchart
# 再次增加flowchart 数据。对于mimic这么复杂的数据，应该没有办法使用flowchart包绘图的方法（只能从一个数据集开始fc_filter这样子，mimic太复杂了，用不了应该）
...
df_gluc_valid <- df_gluc_included_icustays %>% 
  ...
  # 以charttime分组后选取编号最低的项目，编号是根据想要的目的遍的。
  # 查看了数据库，charttime是以分钟为颗粒度的，所以这样可以筛选掉同一分钟内的多个值。
  ...
  # 这个keep_all，其他列的值如果不一样的话是保留哪一行呢？(文档：keeps the first row of values)
...
# 这个比上面好像只是少了一步筛选啊，是不是不太有必要算两遍？还是说还有其他不同？
